...
class GenModelLoss(torch.nn.Module): 
    ...
    def proc_obj(self, obj_verts, obj_params, obj_top_idx=None):
        ...
        if obj_params.shape[-1] == 10 and obj_top_idx is not None:
            ...
            assert obj_params.shape[-1] == 9, f"unexpected obj_params shape: {obj_params.shape}"
            # obj_pc_rotated = torch.einsum("tij,ki->tkj", obj_rotmat, obj_verts)  # grab
            obj_pc_rotated = torch.einsum("tij,kj->tki", obj_rotmat, obj_verts)  # h2o
    ...
    def forward(self, model_output, batch):
        ...
        for batch_offset in range(batch_size):
            ...
            if self.enable_dist_h_loss or self.enable_dist_o_loss or self.enable_penetration_loss or self.enable_contact_loss or self.enable_contact_region_loss:
                ...
                for obj_offset, obj_id in enumerate(obj_list):
                    ...
                    obj_verts_sel = self.proc_obj(obj_verts_can_sel, obj_traj_sel)  # [seq_len, 1024, 3]
        ...
        loss = 0.0
        # baseline
        loss = loss + 0.1 * loss_dist_h  # 手部到物体距离
        loss = loss + 1.0 * loss_rec_joint  # 关节点位置
        loss = loss + 1.0 * loss_rec_vert  # 手部顶点位置
        loss = loss + 1.0 * param_loss  # 没用
        loss = loss + 1.0 * loss_dist_o  # 物体顶点到手部距离
        loss = loss + 0.1 * loss_edge_len  # 边长
        # add
        loss = loss + 1.0 * loss_penetration  # 穿透
        loss = loss + 1.0 * loss_contact  # 手部接触物体
        loss = loss + 0.1 * loss_contact_region   # change from 1.0 to 0.1
        loss = loss + 1.0 * loss_smooth  # 平滑性
        loss_dict = {
            "loss": loss,
