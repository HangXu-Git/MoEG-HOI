...
class GaussianDiffusion:
    ...
    def masked_l2_bh(self, a1, b1, mask1, a2, b2, mask2):
        loss1 = self.masked_l2(a1, b1, mask1)
        loss2 = self.masked_l2(a2, b2, mask2)
        loss = loss1 + loss2
        return loss
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None, loss_callback=None):
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            if type(model_output) == list:     # coarse and fine  单手
                mask = mask.reshape((batch_size, 1, 1, nframes))
                coarse_model_output = model_output[0]
                fine_model_output = model_output[1]
                assert coarse_model_output.shape == target.shape == x_start.shape == fine_model_output.shape  # [bs, njoints, nfeats, nframes]
                terms["mse"] = 1.0 * self.masked_l2(target, coarse_model_output, mask) + 3.0 * self.masked_l2(target, fine_model_output, mask)
                ...
                if "bh" in batch.keys() and batch["bh"] == True:    # 双手
                    assert mask.shape == (batch_size, nframes, 2)
                    model_output_lh = model_output[:, :, 0, :].unsqueeze(2)
                    model_output_rh = model_output[:, :, 1, :].unsqueeze(2)
                    target_lh = target[:, :, 0, :].unsqueeze(2)
                    target_rh = target[:, :, 1, :].unsqueeze(2)
                    mask_lh = mask[:, :, 0].reshape((batch_size, 1, 1, nframes))
                    mask_rh = mask[:, :, 1].reshape((batch_size, 1, 1, nframes))
                    terms["mse"] = self.masked_l2_bh(target_lh, model_output_lh, mask_lh, target_rh, model_output_rh, mask_rh)
                    terms["loss"] = terms["mse"]
                    if loss_callback is not None:
                        extra_loss, extra_loss_dict = loss_callback(model_output, batch)
                    else:
