...
from oakink2_tamf.dataset.arctic_dataset import ARCTIC
...
from ..model.loss_arctic import GenModelLoss
...
def reg_entry(config_reg: ConfigRegistry):
    ...
    config_reg.register(
        ...
        source=ConfigEntrySource.COMMANDLINE_OVER_CONFIG,
        callback=abspath_callback,
        default="data_arctic/arctic/train/data_single_hand.pkl",
    )
    config_reg.register(
        "data_obj_pc_path",
        prefix=PARAM_PREFIX__TRAIN,
        ...
        source=ConfigEntrySource.COMMANDLINE_OVER_CONFIG,
        callback=abspath_callback,
        default="data_arctic/arctic/train/obj.pkl",
    )
    config_reg.register(
        "text_json",
        prefix=PARAM_PREFIX__TRAIN,
        ...
        source=ConfigEntrySource.COMMANDLINE_OVER_CONFIG,
        callback=abspath_callback,
        default="data_arctic/arctic/train/text.json",
    )
    config_reg.register(
        "batch_size",
        prefix=PARAM_PREFIX__TRAIN,
    ...
    config_reg.register(
        ...
        source=ConfigEntrySource.COMMANDLINE_OVER_CONFIG,
        callback=abspath_callback,
        default="data_arctic/arctic/test/data_single_hand.pkl",
    )
    config_reg.register(
        "data_obj_pc_path",
        prefix=PARAM_PREFIX__TEST,
        ...
        source=ConfigEntrySource.COMMANDLINE_OVER_CONFIG,
        callback=abspath_callback,
        default="data_arctic/arctic/test/obj.pkl",
    )
    config_reg.register(
        "text_json",
        prefix=PARAM_PREFIX__TEST,
        ...
        source=ConfigEntrySource.COMMANDLINE_OVER_CONFIG,
        callback=abspath_callback,
        default="data_arctic/arctic/test/text.json",
    )
    config_reg.register(
        "batch_size",
        prefix=PARAM_PREFIX__TEST,
...
def run(rank, world_size, ckpt_cfg, run_cfg):
    ...
    # MoEDiff_multi_group for default
    ...
    if (reload_ckpt_model_filepath := run_cfg["train"]["reload_ckpt_model_filepath"]) is not None:
        ...
        missing_key_list = [k for k in missing_key_list if not k.startswith("blip_model")]
        missing_key_list = [k for k in missing_key_list if not k.startswith("txt_processors")]
    ...
    weight_path = "/data/data0/xuhang/Text2HOI/checkpoints/arctic/pointfeat.pth"
    ...
    for epoch_id in range(now_epoch, num_epoch):
        ...
        for batch_id, batch in enumerate(train_dataloader):
            ...
            batch_device["obj_embedding"] = obj_feat_final.unsqueeze(1)  # (B, 1, 1028)
        ...
        if not rank:
            ...
            if (
                ...
                save_state(model.state_dict(), model_weight_path, remove_prefix="module", filter_out=["clip_model", "long_clip_model", "blip_model", "txt_processors"])
