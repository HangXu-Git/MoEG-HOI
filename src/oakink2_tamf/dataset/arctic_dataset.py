...
class ARCTIC(Dataset):
    def __init__(
        ...
        **kwargs
        ...
        with open(data_path, "rb") as f:
            ...
            self.obj_angle = data["obj_angle"]  # shape (data_num, 238, 1)
    ...
    def __getitem__(self, index):
        ...
        obj_angle = self.obj_angle[index][init_frame:init_frame+self.max_nframes]
        obj_traj = np.concatenate([obj_traj, obj_angle], axis=1)
        ...
        obj_pc_top_idx = np.expand_dims(obj_pc_top_idx, axis=0)  # (1, 1024)
        ...
        res = {
            ...
            "obj_traj": obj_traj,   # (1, nframes, 10)  use obj_num
...
#             self.object_name = data["object_name"]  # list of obj_name, length is 4597
#             self.x_lhand = data["x_lhand"]  # shape is (4597, 238, 99)
#             self.x_rhand = data["x_rhand"]  # shape is (4597, 238, 99)
#             self.x_obj = data["x_obj"]  # shape is (4597, 238, 9)
#             self.x_obj_angle = data["x_obj_angle"]  # shape is (4597, 238, 1)
#             self.lhand_beta = data["lhand_beta"]  # shape is (4597, 238, 10)
#             self.rhand_beta = data["rhand_beta"]  # shape is (4597, 238, 10)
#             print(self.rhand_beta.shape)
#             # augmentation
#             self.lhand_org = data["lhand_org"]
#             self.rhand_org = data["rhand_org"]
#             # interaction info
#             self.is_lhand = data["is_lhand"]  # list of 0/1, length is 4597
#             self.is_rhand = data["is_rhand"]  # list of 0/1, length is 4597
#             self.action_name = data["action_name"]  # list of action name, length is 4597
#             self.nframes = data["nframes"]  # list of nframes, length is 4597
#         with open(text_json, "r") as f:
#             self.text_description = json.load(f)

#         self.object_model = build_object_model(data_obj_pc_path)

#         data_num = self.x_lhand.shape[0]
#         self.info = [f"arctic_{i}_{self.action_name[i]}" for i in range(data_num)]
...
#         pose_repr = np.concatenate([x_lhand, x_rhand], axis=1)  # (150, 198)

#         # hand_shape
#         if is_lhand:
#             lhand_beta = self.lhand_beta[index][init_frame:init_frame+self.max_nframes]
#         else:
#             lhand_beta = np.zeros((150, 10), dtype=np.float32)

#         if is_rhand:
#             rhand_beta = self.rhand_beta[index][init_frame:init_frame+self.max_nframes]
#         else:
#             rhand_beta = np.zeros((150, 10), dtype=np.float32)
#         hand_shape = np.concatenate([lhand_beta, rhand_beta], axis=1)  # (150, 20)
...
#         mask = np.concatenate([valid_mask_lhand, valid_mask_rhand],axis=0) # (2, 150)
...
#             "joint_level_text": joint_level_text,
#             "primitive": action_name,
#             "obj_pointcloud": obj_pc,
#             "obj_pc_normal": obj_pc_normal,
#         }
