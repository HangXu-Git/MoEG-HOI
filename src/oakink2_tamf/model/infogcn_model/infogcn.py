...
class InfoGCNEncoder(nn.Module):
    def __init__(self, num_point=21, num_person=1, num_head=3, graph=None, in_channels=3, out_channels=256,
                 drop_out=0, k=0):
        super(InfoGCNEncoder, self).__init__()
        ...
        self.l2 = EncodingBlock(base_channel, base_channel,A)
        self.l3 = EncodingBlock(base_channel, base_channel,A)
        self.l4 = EncodingBlock(base_channel, base_channel*2, A)
        self.l5 = EncodingBlock(base_channel*2, base_channel*2, A)
        self.l6 = EncodingBlock(base_channel*2, base_channel*2, A)
        self.l7 = EncodingBlock(base_channel*2, base_channel*4, A)
        self.l8 = EncodingBlock(base_channel*4, base_channel*4, A)
        self.l9 = EncodingBlock(base_channel*4, base_channel*4, A)
        bn_init(self.data_bn, 1)
        if drop_out:
            ...
            self.drop_out = lambda x: x
        self.output_process = nn.Linear(base_channel*4, out_channels)
    ...
    def forward(self, x, weight=None):
        ...
        x = self.l7(x)
        x = self.l8(x)
        x = self.l9(x)  # N*M,C,T,V
        x = self.drop_out(x)
        shape = x.shape
        x = x.reshape(N, M, shape[1], shape[2], shape[3]) # N,M,C,T,V
        x = x.permute(0, 2, 3, 4, 1)  # N,C,T,V,M
        x = x.mean(dim=-1) # N,C,T,V
        x = x.permute(0, 2, 3, 1)  # N,T,V,C
        if weight is not None:  # weight: N,V
            x = x * weight.unsqueeze(1).unsqueeze(-1)  # N,T,V,C
            x = x.mean(dim=2)  # N,T,C
        else:
            x = x.mean(dim=2)

        x = self.output_process(x)  # N,T,out_channels
        return x

if __name__ == '__main__':
    model = InfoGCNEncoder(graph='graph.hand.Graph', k=1)
    x = torch.randn(10, 3, 160, 21, 1)
    weight = torch.randn(10, 21)
    out = model(x, weight)
    print(out.shape)
