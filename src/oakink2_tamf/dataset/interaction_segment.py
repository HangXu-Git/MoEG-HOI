...
class InteractionSegmentData(torch.utils.data.Dataset):
        ...
        for process_key in self.process_range_list:
            ...
            for primitive_identifier, primitive_task_data in zip(complex_task_data.exec_path, primitive_task_data_list):
                ...
                for hand_side in HAND_SIDE:
                    ...
                    hand_primitive = primitive_task_data[f"{hand_side}_primitive"]
                    assert hand_primitive is not None, f"hand_primitive is None for {process_key}, {primitive_identifier}, {hand_side}"
                    
                    text_desc = primitive_task_data.task_desc
                    seg_beg, seg_end = primitive_task_data[f"frame_range_{hand_side}"]
                    seg_info = (process_key, primitive_identifier, hand_side, hand_primitive)   # (filename, action:01, lh/rh, hand_primitive) 
                    ...
                    for _offset in range(len(seg_len_sliced)):  # len(seg_len_sliced) = gap
                        _item = {}
                        for obj_id in src_obj_list:
    ...
    def load_object_pointcloud_with_normal(self):
        obj_pointcloud_store = {}
        for obj_id in self.interaction_object_list:
            _filepath = os.path.join(self.obj_pointcloud_prefix, f"{obj_id}.npz")
            with np.load(_filepath) as npz_stream:
                point= npz_stream["point"].astype(np.float32).copy()
                normal = npz_stream["point_normal"].astype(np.float32).copy()
                pointcloud = np.concatenate((point, normal), axis=-1)
            obj_pointcloud_store[obj_id] = pointcloud
        return obj_pointcloud_store
    ...
    def __init__(
        ...
        refine_text_path: Optional[str] = None,
    ):
        self.refine_text_path = refine_text_path
        if self.refine_text_path is not None:
            with open(self.refine_text_path, 'r', encoding='utf-8') as f:
        ...
        # deal with slicing
        self.origin_fps = FPS_MOCAP  # 120
        self.target_fps = target_fps
        self.target_gap = int(self.origin_fps // self.target_fps)  # 12
        ...
        if self.enable_obj_pointcloud:
            self.obj_pointcloud_store = self.load_object_pointcloud_with_normal()
        else:
    ...
    def __getitem__(self, index):
        ...
        info = segment_info[0] + ':' + segment_info[1]
        hand_type = segment_info[2]
        if self.refine_text_dict is not None:
            segment_refine_text = self.refine_text_dict[info][hand_type + '_refine_text']
            ...
            primitive = self.refine_text_dict[info][hand_type + '_primitive']
        else:
            segment_refine_text = None
        ...
        res = {
            ...
            # "refine_text": segment_refine_text,
        ...
        if self.enable_obj_pointcloud:
            ...
            res["obj_pointcloud"] = obj_pointcloud   # (nobj, P, 6) where 6 = xyz + normal
        return res
