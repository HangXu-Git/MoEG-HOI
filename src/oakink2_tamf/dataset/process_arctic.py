    ...
    for index in tqdm.tqdm(range(data_num)):
        for hand_type in ["lhand", "rhand"]:
            if hand_type == "lhand":
                ...
                hand_side = "rh"
            info = f"arctic_{index}_{hand_side}"
            ...
            obj_angle = data["x_obj_angle"][index]
            hand_org = data[f"{hand_type}_org"][index]
            obj_name = data["object_name"][index]
...
change_from_both_hands_to_single(data_path="/data/data4/xuhang/OakInk2-TaMF/data_arctic/arctic/train/data.npz", save_path = "/data/data4/xuhang/OakInk2-TaMF/data_arctic/arctic/test/data_single_hand.pkl")  